...
class Cask:
    # Достаем бочонки по 1 штуке из мешка
    def __init__(self):
        self.amount = 91
        self.gen = self.pull_cask()

    def pull_cask(self):
        __bag = list(range(1, 91))

        for _ in list(range(1, 91)):
            num = random.choice(__bag)
            __bag.pop(__bag.index(num))
            print('{:*^30}'.format('*'))
            print(f'Новый боченок: {num} (осталось {len(__bag)})')
            yield num


class LotoGame:
    # Готовим карточки для игроков
    ...
    def format_card(self, card_player):
    ...
    # Поиск номера в карточке и определение победителя
...
def start_game():
    ...
    while True:
        ...
        player1.format_card(game.card_user)
        player2.format_card(game.card_comp)
